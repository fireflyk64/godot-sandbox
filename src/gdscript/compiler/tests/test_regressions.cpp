...
// Every label a function jumps to is defined in it.
...
static void test_break_after_a_nested_batched_loop() {
	std::cout << "Testing break after a nested batched for loop..." << std::endl;

	// `for x in <Array>` and `for c in <String>` hand over to the batched walk.
	// The generic path had already pushed its own loop context and scope and
	// returned without popping them, so after an inner loop the outer `break`
	// jumped to the inner loop's never-emitted `for_end` label.
	const std::string source = R"(
